...
NutsNews is an automated uplifting-news platform. It discovers story candidates from RSS feeds, uses automation and AI to review them, stores approved stories in Supabase, and displays them through a mobile-friendly Next.js website.
...
The platform can split feeds across many Cloudflare Worker shards. For example:
...
Some deployed Worker/controller files may exist locally before being pushed to GitHub. The intended long-term structure keeps shared logic in one codebase and uses configuration for scaling.

---

## Frontend Web App

The `web` app is the public NutsNews website.

It is built with Next.js and designed around a mobile-first article feed.
...
Each shard runs the same code. Only these variables change:
...
2. Vercel build
   Vercel detects the push, installs dependencies, runs the Next.js build, and prepares production deployment.
...
NutsNews is an active experimental platform for automated uplifting-news discovery, AI-assisted editorial filtering, and mobile-first publishing.
